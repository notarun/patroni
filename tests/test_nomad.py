...
class TestNomadClient(unittest.TestCase):
    ...
    def test_list_and_delete(self):
        self.request.side_effect = [
            self.response(data=b'[{"Path":"service/a"}]'),
            self.response(status=204, data=b'')]
        self.assertEqual([v['Path'] for v in self.client.list_variables('service/')], ['service/a'])
...
class TestNomad(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.c: Any = Nomad({'scope': 'test', 'name': 'postgresql1', 'ttl': 30, 'retry_timeout': 10,
                             'loop_wait': 10, 'host': 'localhost:4646'}, get_mpp({}))
        self.client = Mock(spec=NomadClient)
        self.c._client = self.client
    ...
    def test_recovers_member_lock_after_restart(self):
        data = {'conn_url': 'postgres://localhost/postgres'}
        cluster = Cluster(None, None, None, Mock(), [Member(1, self.c._name, 'existing-lock', data)],
                          None, Mock(), None, None)
        self.c._cluster = cluster
    ...
    def test_leader_lifecycle(self):
        self.client.acquire_lock.return_value = variable(self.c.leader_path, self.c._name, 1, 'leader-lock')
